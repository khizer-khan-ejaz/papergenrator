...
def api_calculate_dms():
    try:
        data = request.get_json()
        P1_lat = dms_to_dd(data.get('P1_lat_deg', '0'), data.get('P1_lat_min', '0'), data.get('P1_lat_sec', '0'), data.get('P1_lat_dir', 'N'))
        P1_lon = dms_to_dd(data.get('P1_lon_deg', '0'), data.get('P1_lon_min', '0'), data.get('P1_lon_sec', '0'), data.get('P1_lon_dir', 'E'))
        P2_lat = dms_to_dd(data.get('P2_lat_deg', '0'), data.get('P2_lat_min', '0'), data.get('P2_lat_sec', '0'), data.get('P2_lat_dir', 'N'))
        P2_lon = dms_to_dd(data.get('P2_lon_deg', '0'), data.get('P2_lon_min', '0'), data.get('P2_lon_sec', '0'), data.get('P2_lon_dir', 'E'))
        P3_lat = dms_to_dd(data.get('P3_lat_deg', '0'), data.get('P3_lat_min', '0'), data.get('P3_lat_sec', '0'), data.get('P3_lat_dir', 'N'))
        P3_lon = dms_to_dd(data.get('P3_lon_deg', '0'), data.get('P3_lon_min', '0'), data.get('P3_lon_sec', '0'), data.get('P3_lon_dir', 'E'))
        P4_lat = dms_to_dd(data.get('P4_lat_deg', '0'), data.get('P4_lat_min', '0'), data.get('P4_lat_sec', '0'), data.get('P4_lat_dir', 'N'))
        P4_lon = dms_to_dd(data.get('P4_lon_deg', '0'), data.get('P4_lon_min', '0'), data.get('P4_lon_sec', '0'), data.get('P4_lon_dir', 'E'))
...
def api_docs():
    docs = {
        ...
        "endpoints": [
            {"path": "/api/calculate", "method": "POST", "description": "Calculate geodesic intersections using decimal degrees", "request_format": {"P1_lat": "float", "P1_lon": "float", "P2_lat": "float", "P2_lon": "float", "P3_lat": "float", "P3_lon": "float", "P4_lat": "float", "P4_lon": "float", "TAS": "float (default: 220)", "wind_speed": "float (default: 50)", "degree": "float (default: 330)", "include_map": "boolean (default: false)"}},
            {"path": "/api/calculate_dms", "method": "POST", "description": "Calculate geodesic intersections using DMS format", "request_format": {"P1_lat_deg": "string", "P1_lat_min": "string", "P1_lat_sec": "string", "P1_lat_dir": "string (N/S)", "P1_lon_deg": "string", "P1_lon_min": "string", "P1_lon_sec": "string", "P1_lon_dir": "string (E/W)", "(same for P2, P3, P4)": "", "TAS": "float (default: 220)", "wind_speed": "float (default: 50)", "degree": "float (default: 330)", "include_map": "boolean (default: false)"}},
            {"path": "/map", "method": "GET", "description": "Render a sample geodesic map using default coordinates"},
            {"path": "/", "method": "GET", "description": "Render the main application interface"}
        ],
        "response_format": {
            "status": "string (success/error)",
            "results": {"p1p2_perp_intersection": {"lat": "float", "lon": "float"}, "nm_line_end_point": {"lat": "float", "lon": "float"}, "perp_nm_p1p2_intersection": {"lat": "float", "lon": "float"}, "p1p2_nm_dist_km": "float", "distance_to_P1_nm": "float", "distance_to_P3_nm": "float", "distance_to_degree": "float", "geojson": "GeoJSON object", "map_html": "HTML string (if include_map=true)"}
        }
    }
    return jsonify(docs)
...
def home():
    # Render the index.html template instead of returning JSON
...
def show_map():
    ...
    P3 = (41.8781, -87.6298)  # Chicago
    P4 = (29.7604, -95.3698)  # Houston
    TAS = 220
    wind_speed = 50
    degree = 330

    results = calculate_geodesic(P1, P2, P3, P4, TAS, wind_speed, degree)
    map_html = results['map_html']
